...
for x in range(1, int(NumberOfJobs)+1):
    ...
    with open('job.sh', 'w') as fout:
        ...
        fout.write("cp readForest.exe tmp/"+str(x)+"/.\n")
        fout.write("cp Summer*.txt tmp/"+str(x)+"/.\n")
        fout.write("cd tmp/"+str(x)+"\n")
        fout.write("pwd\n")
        ...
        fout.write("eval `scramv1 runtime -sh`/n")
        fout.write("export XRD_NETWORKSTACK=IPv4\n")
        fout.write("export X509_USER_PROXY=~/x509_user_proxy/proxy\n")
        fout.write("cd "+str(path)+"\n")
        fout.write("cd tmp/"+str(x)+"\n")
        fout.write("cat list.txt\n")
        fout.write("./readForest.exe list.txt 0 "+str(interval)+" ak 4 PF 0 radialMoment_PythiaEpos_8TeVpthat50_part"+str(x)+".root\n")
