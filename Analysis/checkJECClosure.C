...
void checkJECClosure(int startfile = 2,
		     int endfile = 3,
		     int radius = 4,
		     std::string algo="",
		     std::string jetType= "PF",
		     std::string kFoname="pPbJECCheck.root"){
  ...
  double pthatBins2015[] = {15, 30, 50, 80, 120, 170, 2000};

  infile_Forest = "EPOS8TeV_filelist.txt";
  ...
  for(int ifile = startfile; ifile<endfile; ++ifile){
    ...
    for(int i = 0; i<N; ++i){
      jetpp[i]->Add(filename_Forest.c_str());
      if(printDebug)cout << "Tree loaded  " << string(dir[i]+"/"+trees[i]).data() << endl;
      if(printDebug)cout << "Entries : " << jetpp[i]->GetEntries() << endl;
    }
  ...
  jetpp[1]->SetBranchAddress("pPAprimaryVertexFilter",&pPAprimaryVertexFilter_F);
  ...
  jetpp[4]->SetBranchAddress("lumi",&lumi_F);
  // jetpp[4]->SetBranchAddress("vz",&vz_F);
  jetpp[4]->SetBranchAddress("hiBin",&hiBin_F);
  jetpp[4]->SetBranchAddress("hiHF", &hiHF_F);
  jetpp[4]->SetBranchAddress("hiNpix",&hiNpix_F);
  jetpp[4]->SetBranchAddress("hiNpixelTracks",&hiNpixelTracks_F);
  jetpp[4]->SetBranchAddress("hiNtracks",&hiNtracks_F);
  jetpp[4]->SetBranchAddress("hiNtracksPtCut",&hiNtracksPtCut_F);
  jetpp[4]->SetBranchAddress("hiNtracksEtaCut",&hiNtracksEtaCut_F);
  jetpp[4]->SetBranchAddress("hiNtracksEtaPtCut",&hiNtracksEtaPtCut_F);
  ...
  double pthatweight;
  jetpp[5]->SetBranchAddress("xsec",&pthatweight);
  ...
  for(int nEvt = 0; nEvt < nentries; ++ nEvt) {
	...
	//! get the pp JEC 
	vector<JetCorrectorParameters> vpar_Spring16v8;   
	FactorizedJetCorrector *JEC_Spring16v8 = new FactorizedJetCorrector(vpar_Spring16v8);
	string L2Name;
	string L3Name;
	string L2L3ResName;
	L2Name="Spring16_25nsV8_MC_L2Relative_AK4PF.txt";
	L3Name="Spring16_25nsV8_MC_L3Absolute_AK4PF.txt";    
	L2L3ResName="Spring16_25nsV8_MC_L2L3Residual_AK4PF.txt";    
	...
	JetCorrectorParameters *parFall15_v2_l2l3Res = new JetCorrectorParameters(L2L3ResName.c_str());
	vpar_Spring16v8.push_back(*parFall15_v2_l2);
	vpar_Spring16v8.push_back(*parFall15_v2_l3);
	vpar_Spring16v8.push_back(*parFall15_v2_l2l3Res);
	JEC_Spring16v8 = new FactorizedJetCorrector(vpar_Spring16v8);
	JEC_Spring16v8->setJetEta(eta_F[jet]);
	JEC_Spring16v8->setJetPt(rawpt_F[jet]);
	float jetcorr = JEC_Spring16v8->getCorrection();
	...
	hpp_Trans_matrix_HLT_RecoSmear->Fill( recpt + rnd.Gaus(0,1), genpt, weight_eS * jet40_p_F);rjm
