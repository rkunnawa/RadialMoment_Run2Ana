...
int readForest(std::string fileList,
	       int startfile,
	       int endfile,
	       std::string jetAlgo,
	       int radius,
  ...
  TChain * jetTree[_ntrees];

  for(int t = 0;t<_ntrees;t++){
    jetTree[t] = new TChain(std::string(dir[t]).data());
  }//!tree loop ends
  ...
  for(int ifile = startfile; ifile<endfile; ++ifile){
    ...
    for(int t = 0; t<_ntrees; ++t){
      ...
      if(debugMode)std::cout << "Tree loaded  " << std::string(dir[t]).data() << std::endl;
  ...
  for(int nEvt = 0; nEvt < nentries; ++ nEvt) {
	...
	std::cout<<"    raw jet pT = "<<jet.pt()<<", eta = "<<jet.eta()<<", phi = "<<jet.phi()<<std::endl;
      ...
      JetCorrectorParameters *parFall15_v2_l2l3Res = new JetCorrectorParameters(L2L3Res.c_str());
      vpar_Fall15v2.push_back(*parFall15_v2_l2);
      vpar_Fall15v2.push_back(*parFall15_v2_l3);
      vpar_Fall15v2.push_back(*parFall15_v2_l2l3Res);
      JEC_Fall15v2 = new FactorizedJetCorrector(vpar_Fall15v2);
      JEC_Fall15v2->setJetEta(jet.eta());
      JEC_Fall15v2->setJetPt(jet.pt());
	...
	std::cout<<"    raw jet pT = "<<sd_jet.pt()<<", eta = "<<sd_jet.eta()<<", phi = "<<sd_jet.phi()<<std::endl;
      ...
      JEC_Fall15v2->setJetEta(sd_jet.eta());
      JEC_Fall15v2->setJetPt(sd_jet.pt());
      jetcorr = JEC_Fall15v2->getCorrection();
...
int main(int argc, char *argv[]){
  ...
  if(argc==1)
    ...
    rStatus = readForest(inputFileList,
			 startfile,
			 endfile,
			 jetAlgo,
			 jetRadius,
